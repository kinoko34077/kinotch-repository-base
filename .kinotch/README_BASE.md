...
Base v0.4.0はPhase 5のSurface Default Kit機能releaseである。CLI、Windows、MCP、APIのSurface KitはDomain非依存の薄い補助境界を提供し、既存FrameworkやProject実装はOVERRIDEとして維持できる。Runtime semanticsは変更しない。通常の運用方針は [Phase 5 Operations](meta/07_PHASE5_OPERATIONS.md) を参照する。
